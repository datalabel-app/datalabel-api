...
namespace DataLabeling.BLL
{
    public class EmailService
    {
        ...
        public async Task<bool> SendForgotPasswordEmailAsync(string recipientEmail, string fullName, string password)
        {
            var subject = "Tài khoản Data Labeling của bạn đã được cấp lại mật khẩu";
            var htmlContent = EmailTemplate.GetForgotPasswordEmail(fullName, recipientEmail, password);
            var plainTextContent = $"Xin chào {fullName},\n\nTài khoản của bạn đã được cấp lại mật khẩu.\nEmail: {recipientEmail}\nMật khẩu: {password}\n\nVui lòng đổi mật khẩu của bạn.";
